...
import {saveAvatar} from './dataservice.js'
...
function loadTemplate(urlTemplate,callback,method='GET',params='',cached=true) {
  if (CACHE_TEMPLATES.has(urlTemplate)) {
    ...
    if (method === 'POST') {
      if (urlTemplate === 'api/saveStudents' || urlTemplate === 'api/saveGradedTasks' || urlTemplate === 'api/saveSettings') {
...
function getBase64(file,id) {
  ...
  reader.readAsDataURL(file);
    ...
    reader.onload = function () {
      ...
      let res = reader.result.replace(/^data:image\/jpeg;base64,/, "");
      let a = [];
      a.push(id);
      a.push(res);
...
export {formatDate,popupwindow,hashcode,deleteCookie,setCookie,getCookie,loadTemplate,getIdFromURL,getBase64};
